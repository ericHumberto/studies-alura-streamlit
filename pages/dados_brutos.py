...
# if produtos and colunas.count('Produto') > 0:
#     dados = dados[dados['Produto'].isin(produtos)]

# if preco and colunas.count('Preço') > 0:
#     dados = dados[dados['Preço'].between(preco[0], preco[1], inclusive='both')]

# if data_compra and colunas.count('Data da Compra') > 0:
#     dados = dados[dados['Data da Compra'].between(pd.to_datetime(data_compra[0] if data_compra[0] else dados['Data da Compra'].min(
#     )), pd.to_datetime(data_compra[1] if len(data_compra) >= 2 else dados['Data da Compra'].max()))]
